...
output "routes_ids" {
  value = {
    vm   = concat(aws_route_table.rt_spoke_vm.*.id, aws_route_table.rt_spoke_vm_gwlbe.*.id)[0]
    tgw  = aws_route_table.rt_spoke_tgw_az1.id
    gwlb = aws_route_table.rt_spoke_gwlb.id
    igw  = concat(aws_route_table.rt_spoke_igw.*.id, aws_route_table.rt_spoke_igw_gwlbe.*.id)[0]
  }
}
